...
var nodeRepl = function(opts) {
  ...
  var fs = require("fs");
  var prelude = fs.readFileSync(
    path.dirname(__dirname) + "/lib/prelude.roy",
    "utf8"
  );
  vm.runInNewContext(
  ...
  _.each(argv, function(filename) {
    ...
    if (filename.match(literateExtension)) {
      ...
      console.assert(
        ...
        'Filename must end with ".roy" or ".lroy"'
      );
    }
...
var processFlags = function(argv, opts) {
  ...
  switch (argv[0]) {
    ...
    case "--help":
      ...
      console.log(
        "-s --stdio     : read script from stdin and output to stdout"
      );
...
var main = function() {
  ...
  var opts = {
    info: JSON.parse(
      fs.readFileSync(path.dirname(__dirname) + "/package.json", "utf8")
    ),
    nodejs: true,
    run: false,
    includePrelude: true,
